Real-ESRGAN-TPU 视频/图片分辨率超分应用，通过使用腾讯ARC实验室开源 [Real-ESRGAN](https://github.com/xinntao/Real-ESRGAN) 系列模型，使用 Sophon SDK 移植到 SG2300X 芯片系列产品上，实现本地 TPU 进行硬件加速推理，
...
- 创建虚拟环境
  ...
  **必须创建虚拟环境，否则可能会影响其他应用的正常运行**， 虚拟环境使用请参考[这里](../local-ai-deploy/virtualenv_usage)
